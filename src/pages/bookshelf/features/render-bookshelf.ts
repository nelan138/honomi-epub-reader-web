...
import { getBooksByCategory } from '../../../database/book-repository.ts';
...
function renderBookCard(record: BookRecord): HTMLElement {
   ...
   const progressBarElement = container.querySelector(
      '.progress-bar',
   )! as HTMLElement;
   progressBarElement.style.setProperty(
      '--progress',
      `${Math.random() * 100}%`,
   ); // todo(@me): replace with actual progress value later
...
function renderCategory(
   ...
   const categoryHeaderElement = categoryElement.querySelector(
      '.category-header',
   )!;
   categoryHeaderElement.querySelector('h2')!.textContent = categoryName;

   const categoryActionsElement = categoryHeaderElement.querySelector(
      '.category-actions',
   )!;
   ...
   const expandBtn = categoryActionsElement.querySelector(
      '.expand-category-btn',
   )! as HTMLElement;
   expandBtn.hidden = isExpanded;

   const collapseBtn = categoryActionsElement.querySelector(
      '.collapse-category-btn',
   )! as HTMLElement;
   collapseBtn.hidden = !isExpanded;

   if (!isExpanded) return categoryElement;

   const bookCardListElement = categoryElement.querySelector(
      '.book-card-list',
   )!;
...
export default async function renderBookshelf(searchText = ''): Promise<void> {
   ...
   for (const category of categories) {
      ...
      books = sortAndFilterBooks(
         books,
         searchText,
         userPreferences.titleSortOrder,
      );
